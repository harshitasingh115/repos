...
namespace Travel
{
    public partial class ModifySummary : System.Web.UI.Page
    {
                ...
                using (MySqlCommand cmd = new MySqlCommand("SELECT * FROM tms.travel_summary_hotel WHERE userName ='" + correctPass + "' AND date_completed = 'Not Entered' ORDER BY hotel_date_requested DESC LIMIT  1"))
                {
                    ...
                    using (MySqlDataReader sdr = cmd.ExecuteReader())
                    {
                        if (sdr.Read())
                        {
                            ...
                            ddBedSize.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            DropDownList4.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            txtHotelNumber1.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            txtHotel0.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            txtHotelAddress.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            txtHotelCity.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            DropDownList5.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            txtHotelPostalCode.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            txtCheckInTime.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            txtCheckOutTime.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            txtHotelSubtotal.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            txtHotelHST.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                            txtHotelGrandTotal.Text = sdr[""].ToString() == null ? "N/A" : sdr[""].ToString();
                        }
                    }
                }
        ...
        public void InsertMessage()
        {
            ...
            string Body = Environment.NewLine + "Hello " + lblUser.Text + "!" + Environment.NewLine + Environment.NewLine +
            ...
            "There is no need to ask for anymore insurance as it is already included. Should unfortunate events occur and you need roadside assistance, you can call(844) 556 - 4930 for help(this phone number can also be found on your key tag).";
            msg.Body = Body;
